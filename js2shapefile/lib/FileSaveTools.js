...
(function (preferredOptions) {
    ...
    /**
     * createSaveControl 函数扩展 增加两个参数：isAutoDownload(是否自动下载)，isDownloadZip(是否下载zip格式)
     * @param {*} locationDiv 
     * @param {*} append 
     */
    bH_Proto.createSaveControlExtend = function (locationDiv, append, isAutoDownload, isDownloadZip) {
        ...
        if (_saveMethod == "CHROME") {
            if (isAutoDownload) {
                if (isDownloadZip) {
                    var _base64Content = that._getBase64DataAsSingleFile();
                    if (_base64Content) {
                        location.href = "data:application/zip;base64," + _base64Content;
                    }
                } else {
                    that._saveNative();
                }
